...
class NearbyCrossPlugin : FlutterPlugin, MethodCallHandler {
    ...
    private var userName: ByteArray = "generic_name".toByteArray(Charset.forName("UTF-8"))
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        when (call.method) {
            ...
            "disconnect" -> {
                val serviceId = call.arguments as String
                advertiser?.disconnect(context, serviceId)
                disconnect(context, serviceId)
                result.success(null)
            }
            "sendData" -> {
                val data = call.arguments as String
                advertiser?.sendData(context, data)
                sendData(context, data) // discoverer?.sendData(context, data)
                result.success(null)
            }
